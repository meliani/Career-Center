...
class MasterResearchInternshipAgreementResource extends BaseResource
{
    ...
    protected static ?string $navigationBadgeTooltip = 'Announced Master Research Internship Agreements';

    protected static ?string $navigationGroup = 'Internships and Projects';

    protected static ?string $modelLabel = 'Master Research Internship Agreement';

    protected static ?string $pluralModelLabel = 'Master Research Internship Agreements';

    protected static ?string $title = 'Master Research Internship Agreements';
        ...
        return $table
            ...
            ->actions([
                Tables\Actions\EditAction::make()
                    ->label('')
                    ->visible(fn () => auth()->user()->can('update', new MasterResearchInternshipAgreement)),
            ...
            ->bulkActions([
                Tables\Actions\BulkAction::make('sign')
                    ...
                    ->hidden(fn () => ! auth()->user()->can('sign', new MasterResearchInternshipAgreement))
                ...
                Tables\Actions\BulkActionGroup::make([
                    ...
                    //     ->color('success')
                    //     ->requiresConfirmation()
                    //     ->hidden(fn () => ! auth()->user()->can('validate', new MasterResearchInternshipAgreement))
                    ...
                    //     ->color('warning')
                    //     ->requiresConfirmation()
                    //     ->hidden(fn () => ! auth()->user()->can('achieve', new MasterResearchInternshipAgreement))
                    //     ->action(fn ($records) => $records->each->receive()),
                ])
                    ->hidden(fn () => ! auth()->user()->can('manage', new MasterResearchInternshipAgreement)),
            ])
            ->headerActions([
                \App\Filament\Actions\Action\AssignFinalInternshipsToProjects::make('Assign to Projects')
                    ->hidden(fn () => ! auth()->user()->can('assignToProject', MasterResearchInternshipAgreement::class))
                    ->outlined(),
