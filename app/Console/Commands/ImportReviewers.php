...
class ImportReviewers extends Command implements PromptsForMissingInput
{
    /**
     ...
     * @var string
     */
    protected $signature = 'import:reviewers {filename}';
    ...
    public function handle()
    {
        ...
        while ($data = fgetcsv($handle)) {
            ...
            if ($InternshipAgreement == null) {
                $InternshipAgreement = InternshipAgreement::withoutGlobalScopes()->where('id_pfe', 'like', "%{$importedData['id_pfe']}%")->first();
                if ($InternshipAgreement == null) {
            ...
            $this->ImportProfessorFromInternshipAgreement($project, $importedData['reviewer2']);
            $importedLines++;
        }
